...
class Test extends Model{
    ...
    {
        return $result = $this->db->row('SELECT * FROM `skills`');
    }

    public function ver_m($skills = [])
    {
        if (!empty($skills))
        {
            $results = $skills;
            ...
            return $results = $this->db->row('SELECT * FROM `verification_method` ORDER BY `id`');
        }
    }

    public function getCSV($file_path, $file_encodings = ['cp1251','UTF-8'], $col_delimiter = '', $row_delimiter = "")
    {
